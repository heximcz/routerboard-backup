...
class AdapterDBTest extends RBCaseTest {
	...
	public function testDBAdapterAddIP() {
		...
		for ($i=100; $i<120; $i++) {
			$this->assertTrue( $db->addIP( '192.168.1.' . $i, '2345', 'RB-Test' . $i ) );
		}
	}
	
	public function testDBAdapterUpdateIP() {
		$db = new RouterBoardDBAdapter(self::$config, new OutputLogger( new NullOutput() ));
		$this->assertTrue( $db->updateIP( '192.168.1.100', '192.168.1.50', '2345', 'RB-Test50' ) );
		$this->assertFalse( $db->updateIP( '192.168.1.70', '192.168.1.71', '2345', 'RB-Test111' ) );
	}
